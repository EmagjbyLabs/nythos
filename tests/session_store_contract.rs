...
fn contract_supports_revoke_one_and_tenant_scoped_revoke_all() {
    block_on(async {
        ...
        store.revoke_session(session_a1.id()).await.unwrap();

        assert!(
            store
                .find_by_refresh_token(&refresh_a1)
                .await
                .unwrap()
                .is_none()
        );

        store.revoke_all_for_user(tenant_a, user_id).await.unwrap();

        assert!(
            store
                .find_by_refresh_token(&refresh_a2)
                .await
                .unwrap()
                .is_none()
        );
